...
const AdminProductListSections = () => {
  ...
  const loadSections = async () => {
    ...
    try {
      ...
      setFeedback({
        type: "error",
        message: getApiErrorMessage(error, "Failed to load product list sections."),
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleExport = () => {
    const rowsToExport =
      selection.selectedCount > 0
        ...
        : sortedRows;
    exportListToCsv({
      filename: `product-list-sections-${new Date().toISOString().slice(0, 10)}.csv`,
    ...
    <Box sx={{ minHeight: "100dvh", bgcolor: pageBg, boxSizing: "border-box" }}>
      ...
      <Box sx={{ maxWidth: 1120, mx: "auto", px: { xs: 2, sm: 3 }, py: 3 }}>
        ...
        <AdminListTable
          ...
          emptyMessage="No product list sections found."
          selectable
        ...
        >
          {sortedRows.map((row, index) => {
            ...
            <TableRow
              ...
              onClick={() => rowId && navigate(`/admin/homepage-cms/product-list-sections/${encodeURIComponent(rowId)}`)}
            >
              <AdminRowSelectCell
                checked={checked}
                disabled={!rowId}
